fid = fopen('woman_EMG\womanok.txt', 'r');
nlines = 851; %to chenage before execute
nofframe = 50;  %to chenage before execute
...
fileID = fopen('woman_EMG\womanraw.txt','w');
...
fileid = fopen('woman_EMG\womanraw.txt');
P = textscan(fileid,'%d %s %f %f %f');

%create a new .trc file following the formatting rules
fileIDtrc = fopen('woman_EMG\woman.trc','w');
fprintf(fileIDtrc,'PathFileType\t4\t(X/Y/Z)\twoman.trc\t\n');
